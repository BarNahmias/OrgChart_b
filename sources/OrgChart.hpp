                ...
                while (node->next!= nullptr || node->child != nullptr)
                {
                    if (node->next != nullptr)
                    {
                        if(node->parent->child==node){
                            ...
                            node->next->perv = nullptr;
                            node->next= nullptr;

                        }
                        if(node->perv!= nullptr){
                            delete_nodes(node->child);
...
//    public:
        ...
        reverseLevelIterator<T> begin_reverse_order()
        {
            return reverseLevelIterator<T>{ root};
        }
        reverseLevelIterator<T> reverse_order()
        {

            return reverseLevelIterator<T>{};
        }
